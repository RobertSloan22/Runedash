...
const RunePricePrediction = () => {
  ...
  const fetchRuneNames = async () => {
    try {
      const response = await axios.get('https://nodeendpoint.ngrok.app/api1/rune-names');
  ...
  const fetchData = async () => {
    ...
    const requestUrl = `https://nodeendpoint.ngrok.app/api1/rune-data?rune_name=${encodeURIComponent(ticker)}&limit=10`;
  ...
  const trainModel = async () => {
    ...
    const history = await newModel.fit(X, y, {
      epochs: 10,
      batchSize: 32,
      validationSplit: 0.2,
      callbacks: {
  ...
  return (
    <div style={styles.container}>
      ...
      <div>
        ...
        <div id="load_fetch_data" style={styles.loadingIndicator}>
          Loading...
        </div>
      </div>
      ...
      {smaData.length > 0 && (
        <div id="div_container_getsma">
          ...
          <div id="load_draw_sma" style={styles.loadingIndicator}>
            Loading...
          </div>
        </div>
      )}
