...
const RunePricePrediction = () => {
  ...
  const fetchData = async () => {
    ...
    const requestUrl = `https://nodeendpoint.ngrok.app/api1/rune-data?rune_name=${encodeURIComponent(ticker)}&limit=100`;
    ...
    <div>
      ...
      <div>
        <select onChange={(e) => setTicker(e.target.value)}>
          {runeNames.map(rune => (
            <option key={rune.rune_name} value={rune.rune_name}>{rune.rune_name}</option>
          ))}
        </select>
        <input
          type="text"
